...
export class MylearingScreenComponent implements OnInit {
    ...
    this.courseService
      ...
      .subscribe((enrollmentData) => {
        this.courseService.getAllCourse().subscribe((courseData) => {
          ...
          this.myCourseList = this.myCourseList.sort((a, b) => {
            return (
              <any>new Date(b.purchasedAt ? b.purchasedAt : b.createdAt) -
              <any>new Date(a.purchasedAt ? a.purchasedAt : a.updatedAt)
            );
          });
  ...
  searchCourse(input: string) {
    this.searchedCourses = this.myCourseList.filter((course) =>
